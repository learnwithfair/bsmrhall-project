<?php if ( isset( $_POST['prepdf'] ) ) {
    ...
    $obj3 = new pdf_print();
    $roll = $_SESSION['check_pre_r_roll'];

    // Get Pre Residential Info
    $display_r_query = "SELECT * FROM residential_info WHERE (non_r_roll=$roll)";
    $display_r_info  = mysqli_query( $obj3->preconn, $display_r_query );

    // Get Pre Residential Payment Info
    ...
    $info_title   = array( "Full Name", "Roll Number", "Registration No", "Hall ID", "Session", "Date of Birth", "Admission Date", "Ending Date", "Room No", "Mobile No", "Email", "Name of the Department", "Father's Name", "Mother's Name", "Present Address", "Permanent Address" );
    $info_heading = array( "non_r_name", "non_r_roll", "non_r_reg", "non_r_hall_id", "non_r_session", "non_r_birth", "non_r_start", "non_r_end", "non_r_rm", "non_r_mob", "non_r_email", "non_r_dept", "non_r_fname", "non_r_mname", "non_r_pre_address", "non_r_per_address" );
